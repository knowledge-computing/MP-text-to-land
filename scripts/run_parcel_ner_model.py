...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str, default="./src/models/state_county_city_ner_model/model-best",
                        help='To identify parcel numbers, currently load the same trained NER model for state, county, and city. Load it from this path')

    parser.add_argument('--output_path', type=str, default="./output/parcel_w_txt_n_image_ids.jsonl",
...
def extract_state_cnty_cty_kv(doc):
    # result = {"SUBDIVISION": []}
    result = {"LOT": [], "BLOCK": [], "UNIT": [], "TOWNSHIP": [], "RANGE": [], "SECTION": [], "QUARTER": []}
    for ent in doc.ents:
...
def main():
    ...
    for item in result_list:
        ...
        for text in item['sentences']:
            ...
            # print(text)
            # print(kv)
            json_line = json.dumps({"text": text, "image_ids": item['image_ids'], "NERpredicted_LOT": kv['LOT'], "NERpredicted_BLOCK": kv['BLOCK'], "NERpredicted_UNIT": kv['UNIT'], "NERpredicted_TOWNSHIP": kv['TOWNSHIP'], "NERpredicted_RANGE": kv['RANGE'], "NERpredicted_SECTION": kv['SECTION'], "NERpredicted_QUARTER": kv['QUARTER']}, ensure_ascii=False)
